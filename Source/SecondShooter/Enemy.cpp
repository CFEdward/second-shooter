...
AEnemy::AEnemy() :
	...
	BaseDamage(20.f),
	LeftWeapHitParticleSocket(TEXT("FX_Clang_L")),
	RightWeapHitParticleSocket(TEXT("FX_Clang_R")),
	bCanAttack(true),
	AttackWaitTime(1.f),
	bDying(false),
	DeathTime(4.f)
...
void AEnemy::BeginPlay()
{
	...
	// Ignore the camera or mesh and capsule
...
void AEnemy::AggroSphereOverlap(
...
void AEnemy::CombatRangeBeginOverlap(
	UPrimitiveComponent* OverlappedComponent,
	AActor* OtherActor,
	UPrimitiveComponent* OtherComp,
	int32 OtherBodyIndex,
	...
	const FHitResult& SweepResult)
{
	...
	if (ShooterCharacter)
	{
		...
		if (EnemyController)
		{
			EnemyController->GetBlackboardComponent()->SetValueAsBool(TEXT("bInAttackRange"), true);
		}
	}
}
	...
	int32 OtherBodyIndex)
{
	...
	if (ShooterCharacter)
	{
		...
		if (EnemyController)
		{
			EnemyController->GetBlackboardComponent()->SetValueAsBool(TEXT("bInAttackRange"), false);
		}
	}
}
	...
	const FHitResult& SweepResult)
{
	...
	{
		DoDamage(Character);
		SpawnBlood(Character, LeftWeapHitParticleSocket);
	...
	const FHitResult& SweepResult)
{
	...
	{
		DoDamage(Character);
		SpawnBlood(Character, RightWeapHitParticleSocket);
		StunCharacter(Character);
	}
}
...
{
	const USkeletalMeshSocket* ClangSocket{ GetMesh()->GetSocketByName(SocketName) };
	if (ClangSocket)
	{
		const FTransform SocketTransform{ ClangSocket->GetSocketTransform(GetMesh()) };
...
void AEnemy::SetStunned(bool Stunned)
{
	...
	if (EnemyController)
	{
		EnemyController->GetBlackboardComponent()->SetValueAsBool(TEXT("bStunned"), Stunned);
	}
}
